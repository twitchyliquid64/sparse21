...
use crate::sperror::{sperror, TestResult};
...
impl Assert<&Vec<f64>> {
    ...
    pub fn constant(&self, val: f64) -> TestResult {
        for k in 0..self.val.len() {
            if self.val[k] != val {
                return raise(format!(
                    "Non-constant vector with value {} at index {}",
                    self.val[k], k
                ));
            }
        }
...
impl Assert<&HashMap<String, Vec<f64>>> {
    pub fn isclose(&self, other: HashMap<String, Vec<f64>>, tol: f64) -> TestResult {
        ...
        for key in self.val.keys() {
            ...
            if s.len() != o.len() {
                return raise(format!(
                    "Mismatched Vector Lengths {} vs {} for Signal {}",
                    s.len(),
                    o.len(),
                    key
                ));
